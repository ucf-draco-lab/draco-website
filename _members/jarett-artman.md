---
name: Jarett Artman
image: images/people/jarett-artman.jpg
role: ms-alumni
date: 2025-05-01
degrees:
  - level: M.S.
    year: 2025
sponsors: [ucf]
links:
  home-page: http://jarettartman.com
  linkedin: jarett-artman
---

Jarett Artman earned his master's degree at the University of Central Florida, specializing in machine learning and artificial intelligence. His research focused on applying genetic algorithms to time series data, with a particular emphasis on financial portfolio management. Throughout his academic journey, Jarett has contributed to a range of innovative projects, including 'Auto Caddie,' an AI-powered tool that provides real-time swing analysis for golfers, and AI-based weather forecasting models aimed at improving farming practices. With hands-on industry experience Jarett is passionate about advancing AI research and applying it to solve complex, real-world challenges.
